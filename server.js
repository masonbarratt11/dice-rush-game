// server.js - DICE RUSH with Real Dice Graphics
...
app.get('/game', (req, res) => {
  ...
  const html = '<!DOCTYPE html><html><head><title>DICE RUSH</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>*{margin:0;padding:0;box-sizing:border-box}body{background:linear-gradient(135deg,#0f5f2f,#1a8c4d);color:white;font-family:Arial,sans-serif;padding:16px;min-height:100vh}.container{max-width:500px;margin:0 auto}h1{text-align:center;font-size:28px;margin-bottom:16px}h2{font-size:20px;text-align:center}.card{background:rgba(0,0,0,0.3);padding:16px;border-radius:8px;margin-bottom:12px;border:1px solid #00ff00}.balance{font-size:20px;font-weight:bold;color:#ffd700}button{width:100%;padding:12px;background:linear-gradient(135deg,#ff6b00,#ff8800);color:white;border:none;border-radius:6px;cursor:pointer;font-weight:bold;margin-bottom:8px;font-size:14px}button:hover{opacity:0.9}button.bot-btn{background:linear-gradient(135deg,#7c3aed,#a855f7)}.input{width:100%;padding:10px;background:#1a1a2e;border:1px solid #ffd700;color:white;border-radius:4px;margin-bottom:8px;font-size:14px}.hidden{display:none}.match-info{text-align:center;margin:16px 0}.round{text-align:center;margin:12px 0;padding:12px;background:rgba(0,0,0,0.5);border-radius:4px}.dice-container{display:flex;justify-content:center;align-items:center;gap:20px;margin:20px 0}.dice{width:90px;height:90px;background:linear-gradient(135deg,#f5f5f5,#ffffff);border:2px solid #333;border-radius:6px;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);padding:6px;gap:3px}.pip{width:14px;height:14px;background:#ff0000;border-radius:50%}.pip.hidden{background:transparent}.vs{font-size:18px;color:#ffd700;font-weight:bold}.round-title{font-size:12px;color:#ffd700;margin-bottom:8px}</style></head><body><div class="container"><h1>🎲 DICE RUSH</h1><div id="setupScreen" class="setup"><div class="card"><div style="font-size:12px;color:#888;margin-bottom:4px;">Balance</div><div class="balance" id="balance">$100.00</div></div><div class="card"><div style="font-size:12px;color:#ffd700;margin-bottom:8px;font-weight:bold;">Set Bet Amount</div><input type="number" id="betAmount" class="input" value="5" min="1" placeholder="Enter bet amount"><button onclick="findOpponent()">🎯 Find Opponent</button><button class="bot-btn" onclick="playBot()">🤖 Play Bot</button></div></div><div id="gameScreen" class="gameplay hidden"><div class="card match-info"><h2 id="matchStatus">Match: 0-0</h2><div style="font-size:14px;color:#ffd700;margin-top:8px;"><span id="player1Name">You</span> vs <span id="player2Name">Opponent</span></div><div style="font-size:12px;color:#888;margin-top:8px;">Pot: $<span id="potAmount">0</span></div></div><div id="roundDisplay" class="card"><div class="round-title">Round <span id="roundNum">1</span>/3</div><div class="dice-container"><div class="dice" id="p1Dice"></div><div class="vs">vs</div><div class="dice" id="p2Dice"></div></div><div id="roundResult" style="margin-top:12px;color:#00ff00;font-weight:bold;"></div></div><button id="rollBtn" onclick="rollDice()">🎲 Roll Dice</button><button id="nextBtn" onclick="nextRound()" class="hidden">→ Next Round</button><div id="matchResult" class="card hidden" style="text-align:center;"><h2 id="resultText"></h2><div id="prizeText" style="margin-top:8px;font-size:16px;color:#ffd700;"></div><button id="playAgainBtn" onclick="playAgain()" class="bot-btn">🎮 Play Again</button></div></div></div><script>const API_BASE="' + apiBase + '";let telegramId=Math.random().toString();let matchId=null;let currentMatch=null;let isBot=false;function getDicePips(num){const pips=[false,false,false,false,false,false,false,false,false];if(num===1){pips[4]=true;}else if(num===2){pips[0]=true;pips[8]=true;}else if(num===3){pips[0]=true;pips[4]=true;pips[8]=true;}else if(num===4){pips[0]=true;pips[2]=true;pips[6]=true;pips[8]=true;}else if(num===5){pips[0]=true;pips[2]=true;pips[4]=true;pips[6]=true;pips[8]=true;}else if(num===6){pips[0]=true;pips[2]=true;pips[3]=true;pips[5]=true;pips[6]=true;pips[8]=true;}return pips;}function renderDice(elementId,num){const pips=getDicePips(num);let html="";for(let i=0;i<9;i++){html+=\'<div class="pip\'+(pips[i]?"":" hidden")+"\" ><\/div>";}document.getElementById(elementId).innerHTML=html;}async function findOpponent(){const betAmount=parseFloat(document.getElementById("betAmount").value);const apiUrl=API_BASE+"/api/match/find";try{const response=await fetch(apiUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({telegramId:telegramId,username:"Player",betAmount:betAmount})});const data=await response.json();if(data.matchId){matchId=data.matchId;isBot=false;startGame(data);}else{alert(data.message||"Searching for opponent...");}}catch(error){alert("Error: "+error.message);}}async function playBot(){const betAmount=parseFloat(document.getElementById("betAmount").value);const apiUrl=API_BASE+"/api/match/vs-bot";try{const response=await fetch(apiUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({telegramId:telegramId,username:"Player",betAmount:betAmount})});const data=await response.json();if(data.matchId){matchId=data.matchId;isBot=true;startGame(data);}else{alert("Error: "+(data.error||"Could not start bot match"));}}catch(error){alert("Error: "+error.message);}}function startGame(data){document.getElementById("setupScreen").classList.add("hidden");document.getElementById("gameScreen").classList.remove("hidden");document.getElementById("player1Name").textContent="You";document.getElementById("player2Name").textContent=data.opponent;document.getElementById("potAmount").textContent=data.pot;loadMatch();}async function loadMatch(){try{const response=await fetch(API_BASE+"/api/match/"+matchId);currentMatch=await response.json();updateDisplay();}catch(error){console.error("Error loading match:",error);}}async function rollDice(){const roll=Math.floor(Math.random()*6)+1;try{const response=await fetch(API_BASE+"/api/match/"+matchId+"/roll",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({telegramId:telegramId,roll:roll})});const data=await response.json();currentMatch=data.match;updateDisplay();if(data.matchComplete){showResult(data);}else{document.getElementById("rollBtn").classList.add("hidden");document.getElementById("nextBtn").classList.remove("hidden");}}catch(error){alert("Error: "+error.message);}}function nextRound(){document.getElementById("rollBtn").classList.remove("hidden");document.getElementById("nextBtn").classList.add("hidden");renderDice("p1Dice",0);renderDice("p2Dice",0);document.getElementById("roundResult").textContent="";}function updateDisplay(){const lastRound=currentMatch.rounds[currentMatch.rounds.length-1];if(lastRound){document.getElementById("roundNum").textContent=lastRound.round;if(lastRound.p1Roll){renderDice("p1Dice",lastRound.p1Roll);}if(lastRound.p2Roll){renderDice("p2Dice",lastRound.p2Roll);}if(lastRound.winner===1){document.getElementById("roundResult").textContent="✓ You won this round!";}else if(lastRound.winner===2){document.getElementById("roundResult").textContent="✗ Opponent won this round";}else{document.getElementById("roundResult").textContent="= Tie!";}const p1Wins=currentMatch.player1Wins||0;const p2Wins=currentMatch.player2Wins||0;document.getElementById("matchStatus").textContent="Match: "+p1Wins+"-"+p2Wins;}}function showResult(data){document.getElementById("rollBtn").classList.add("hidden");document.getElementById("nextBtn").classList.add("hidden");document.getElementById("roundDisplay").classList.add("hidden");document.getElementById("matchResult").classList.remove("hidden");const result=data.winner===1?"🎉 YOU WON!":"😢 YOU LOST";document.getElementById("resultText").textContent=result;document.getElementById("prizeText").textContent=data.winner===1?"Prize: $"+data.prize:"Better luck next time!";document.getElementById("balance").textContent="$"+data.newBalance;}function playAgain(){document.getElementById("setupScreen").classList.remove("hidden");document.getElementById("gameScreen").classList.add("hidden");document.getElementById("roundDisplay").classList.remove("hidden");document.getElementById("matchResult").classList.add("hidden");matchId=null;currentMatch=null;}</script></body></html>';
  res.send(html);
});
